...
//
// Los cuatro van en CAMPOS CON NOMBRE y no embebidos: `Members`, `Roles` y `Sessions` tienen los tres
// un `List`, y embebiéndolos la llamada `api.List(...)` sería un selector ambiguo que ni siquiera
// compila. Con nombre, además, la llamada dice el plano al que va —`api.Roles.Assign(…)` sobre una
// ruta que empieza por /members— que es justo la distinción que el scope hace y el prefijo esconde.
type Client struct {
