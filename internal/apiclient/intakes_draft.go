...
// Clases de revisión que publica el detalle (`intake_revisions.kind`). Van como constantes porque la
// pantalla elige POR CLASE cuál pinta, y una errata en un literal repartido por tres ficheros no la
// detecta el compilador.
//
// El histórico se lee, no se juzga: una clase que este cliente no conozca llega igual dentro de
// IntakeDetail.Revisions y se pinta con su nombre crudo — antes una clave sin traducir que un
// histórico con huecos.
const (
...
// guarda una forma distinta —la `interpreted` es el borrador rico del LLM, la `corrected` es la lista
// plana de líneas que mandó el dueño— y decodificarlas todas aquí obligaría a este cliente a conocer
// siete contratos para pintar uno. Quien necesite el de la interpretación llama a
...
type IntakeRevision struct {
	...
	// (D-044.52 §3). Va como PUNTERO y no como string porque la pregunta que decide la pantalla es de
	// PRESENCIA DE CLAVE, no de valor:
	//
	...
	//
	// Con un `string` pelado la clave ausente y un `""` explícito colapsarían en el mismo cero, y las
	// dos ramas de arriba dirían lo mismo. Es hermano de `created_at` y vive FUERA de `payload`.
	//
...
// Clases de adjunto de una IntakeMediaRef (`anclaje.Kind*` de la plataforma). Los tres primeros son
// AUDIO: `ptt` es la nota de voz de WhatsApp y `voice` el alias que usan algunos puentes.
const (
...
// IsAudio responde si el adjunto se escucha. Es lo que decide el rótulo del borrador, y por eso cubre
// los tres alias: tratar `ptt` como «otro adjunto» dejaría la nota de voz —que es la forma más
// habitual en que un cliente pide— sin mención en la pantalla.
...
//
// 🔴 NO ES IntakeItem, y confundirlas es el error que la pantalla del borrador existe para no
// cometer. IntakeItem son las líneas RESUELTAS —las que la plataforma factura— y su `unit_price` es
// un `float64` que no puede decir «todavía no hay precio»; la línea `unmatched`, que es justo la que
...
// `0` significa «va de regalo». Un `float64` pelado colapsaría las dos en `0.00`.
...
// Price es el precio unitario, y 0 cuando no hay. Solo se llama detrás de HasPrice: quien lo use sin
// preguntar imprimirá un 0 que significa «gratis» sobre una línea que no tiene precio.
...
// IntakeInterpretation es el payload de una revisión `interpreted`: el borrador que la pantalla
// pinta.
//
// `SuggestedQuestions` es PUNTERO y ahí hay una distinción que la pantalla necesita: la lista vacía
// (`[]`) significa «no había nada que preguntar», y la clave AUSENTE significa que la empresa no
// tiene la feature `llm_intake` —la plataforma borra la clave, no la deja en `[]`, justamente para
// que las dos no se confundan—. Colapsarlas en un slice nil le diría al dueño que el LLM no tenía
// preguntas cuando lo que pasa es que no ha pagado por ellas.
...
// QuestionsKnown responde si la plataforma llegó a publicar la clave. Falso ⇒ la empresa no tiene
...
// UNA de las siete clases: decodificar el payload de una `corrected` con esta forma daría un borrador
// con las líneas vacías y nadie se enteraría.
//
// NO usa `DisallowUnknownFields`, como ningún decodificador de este cliente: es lo que permite que
// esta consola siga funcionando cuando la plataforma añada un campo, en vez de caerse entera por algo
// que no iba a pintar.
...
//
// «Última» es la de mayor `revision_no` y no la última del slice: el orden con el que la API devuelve
// el histórico no es contrato, y ordenar por posición dejaría al dueño mirando una interpretación
// vieja el día que ese orden cambie.
...
// LinesWithoutPriceError es el 400 `lines_without_price`: quedan líneas sin precio y la cotización no
// puede salir. Trae TODAS de una vez, que es lo que el dueño necesita para arreglarlas en una pasada.
//
// Lo emiten DOS puertas con el mismo cuerpo —`/approve` y `/quote-suggestion`—, porque es la misma
...
func (e *LinesWithoutPriceError) Unwrap() error { return ErrInvalidInput }
...
// NotApprovableError es el 422 `not_approvable`: la solicitud no está en un estado que admita la
// aprobación. Trae dónde está AHORA y desde qué estados SÍ se aprueba, por lo mismo que
// NotEditableError: el ciclo de vida lo manda la plataforma y esta consola no lo replica.
...
func (e *NotApprovableError) Unwrap() error { return ErrInvalidInput }
...
//
// El texto es el ÚNICO campo, y es obligatorio (D-044.19): el dueño es el autor de lo que sale, y lo
// que la revisión guarda es byte a byte lo que se envió. No van aquí ni las líneas ni el total —esos
// ya están escritos— y mandarlos abriría la puerta a aprobar una cotización distinta de la que está
// en la solicitud.
...
// EDITADA por el dueño. Las `suggested_questions` del borrador son una propuesta y jamás salen solas
// (INV-1); lo que viaja es lo que el dueño dejó escrito.
...
// WhatsApp cuelga de una sesión que puede estar caída, y prometerle al dueño la entrega es prometerle
// algo que esta puerta no sabe.
//
// Errores: *LinesWithoutPriceError (400, con la lista entera), *NotApprovableError (422),
// *InvalidTransitionError (422 por carrera con otro operador), ErrIntakeChanged (409),
// *FeatureNotEnabledError (403) y ErrInvalidInput para el 400 sin clave —el texto vacío—.
func (c *IntakesClient) ApproveIntake(ctx context.Context, accessToken, id, renderedText string) (*IntakeDetail, error) {
	return c.intakeAction(ctx, accessToken, id, "approve", "intakes.approve",
...
// lo tenga delante.
//
// Errores: ErrInvalidInput para el 400 (pregunta vacía), *InvalidTransitionError para el 422 y
// ErrIntakeChanged para el 409. Esta puerta NO emite `not_approvable` ni `lines_without_price`.
func (c *IntakesClient) RequestIntakeInfo(ctx context.Context, accessToken, id, question string) (*IntakeDetail, error) {
	return c.intakeAction(ctx, accessToken, id, "request-info", "intakes.request-info",
...
func (c *IntakesClient) intakeAction(ctx context.Context, accessToken, id, action, op string, payload any) (*IntakeDetail, error) {
	req, err := c.t.newAuthedRequest(ctx, http.MethodPost,
		"/api/v1/intakes/"+pathSegment(id)+"/"+action, payload, accessToken)
	if err != nil {
		return nil, err
	}
	resp, err := c.t.doTyped(req, op, intakeActionError)
	if err != nil {
		return nil, err
	}
...
// intakeActionError traduce un no-2xx de las acciones del detalle. Decide por la CLAVE `error` del
// cuerpo y no por el código, porque dos rechazos distintos comparten status: el 400 puede ser «faltan
// precios» o «falta el texto», y el 422 puede ser «este estado no aprueba» o «otro operador la movió»
// —y los dos 422 llevan a consejos opuestos: uno es cambiar de estado, el otro es recargar—.
func intakeActionError(op string, resp *http.Response) error {
	...
	case http.StatusUnprocessableEntity:
		...
		if body.Error == errInvalidTransition {
