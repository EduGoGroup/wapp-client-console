...
const (
	// QuoteSourceLLM — el texto lo redactó el modelo con el estilo de la empresa.
	QuoteSourceLLM = "llm"
...
// cuatro los emite el generador (`quotetext/quotetext.go`) y NUEVE el verificador de precios
// (`quotetext/precios.go`), que viajan por ESTE MISMO campo. Un catálogo a medias no rompe nada
// visible: el motivo que falte se pinta con su clave cruda, que es exactamente la fuga que el test de
// la pantalla existe para cazar.
//
// Van declarados aquí y no importados porque esta consola NO depende del cloud (son dos módulos
// separados): lo que hay es una copia con la fuente escrita al lado.
const (
	// — Los CUATRO del generador —

	// QuoteFallbackNoExamples — la empresa no tiene ni historial aprobado ni semilla de estilo, así
	...
	// QuoteFallbackProviderDown — no se pudo obtener el proveedor de la vía de la empresa.
	...
	// QuoteFallbackDraftWithoutAmounts — el borrador no tiene ni un importe positivo. Es el único que
	// se emite por DOS caminos: también antes de llamar al modelo, cuando todas las líneas están por
	// confirmar.
	...
	// QuoteFallbackTextWithoutAmounts — el texto no trae NI UN importe: no dice precios, así que no es
	// una cotización.
	...
	// QuoteFallbackForeignNumber — el texto trae un número sin marca de moneda, por encima de todo lo
	// que cuesta el pedido, que tampoco sale del borrador.
	QuoteFallbackForeignNumber = "numero_ajeno"
	// QuoteFallbackAmountsOutOfPlace — los importes salen del borrador pero no están donde les toca:
	// sobran, faltan o van en otro orden.
	QuoteFallbackAmountsOutOfPlace = "importes_fuera_de_sitio"
)

// IntakeQuoteSuggestion es el 200 de POST /api/v1/intakes/{id}/quote-suggestion (Plan 044 · T5.1): la
// cotización REDACTADA que se le propone al dueño, y la verdad sobre quién la escribió.
//
...
// su FallbackReason. Quien lo pinte no debe tratar `deterministic` como un fallo — es una cotización
// utilizable con otra procedencia. Por eso aquí no hay rama para 502/503: no existen.
type IntakeQuoteSuggestion struct {
	// RenderedText es la cotización sugerida. Se llama IGUAL que el campo del cuerpo de `approve` a
	// propósito: es el texto que la consola copia de una respuesta al siguiente formulario.
...
//
// NO LLEVA CUERPO, y eso es el contrato: todo lo que hace falta está en el token (la empresa) y en la
// ruta (la solicitud). El cloud ni siquiera lee el cuerpo.
//
// 🔴 ES LA ÚNICA LLAMADA DE ESTA CONSOLA QUE VA POR doInference, y tiene que seguir siéndolo: es la
// única que espera a que un modelo redacte. El cliente general corta a los 15s y esta llamada tardó
// 24,8 / 28,4 / 29,7 / 35,5 segundos medida contra UAT el 2026-08-28, así que por ahí no cabe.
// Cambiarla por doTyped devuelve el fallo que este plazo existe para quitar; usar doInference en otra
// llamada le regala 55s a algo que no los necesita. Hay un test estructural que cuenta las
// apariciones del selector: si aparece en otro sitio, falla.
//
// Errores: *LinesWithoutPriceError (400 `lines_without_price` — el MISMO cuerpo que `approve`, porque
// es la misma precondición sobre el mismo objeto), *FeatureNotEnabledError (403, y aquí la capacidad
// que falta puede ser `cart_basic` o `llm_intake`: la ruta lleva los dos gates encadenados),
// ErrInvalidInput para el otro 400 —la solicitud no tiene líneas que cotizar— y los generales para
// 404/5xx. NO emite 422 ni 409.
func (c *IntakesClient) SuggestIntakeQuote(ctx context.Context, accessToken, id string) (*IntakeQuoteSuggestion, error) {
	const op = "intakes.quote-suggestion"
	req, err := c.t.newAuthedRequest(ctx, http.MethodPost,
		"/api/v1/intakes/"+pathSegment(id)+"/quote-suggestion", nil, accessToken)
	if err != nil {
		return nil, err
	}
	resp, err := c.t.doInference(req, op, quoteSuggestionError)
	if err != nil {
		return nil, err
	}
...
// quoteSuggestionError traduce un no-2xx del generador. Los DOS cuerpos de 400 piden consejos
// distintos —arreglar precios frente a añadir líneas— y solo los separa la clave `error`.
func quoteSuggestionError(op string, resp *http.Response) error {
	...
	if resp.StatusCode == http.StatusBadRequest && body.Error == errLinesWithoutPrice {
